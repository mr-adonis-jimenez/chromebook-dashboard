...
An interactive **sales forecasting dashboard** built entirely on a **Chromebook** using Python and Streamlit.  
The app forecasts future sales for multiple Google hardware products (Chromebook, Pixelbook, etc.) using the **Prophet** model, supports **auto-refresh retraining**, and integrates seamlessly with **Google Drive (local sync)**.
